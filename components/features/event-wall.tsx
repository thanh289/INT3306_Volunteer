...
export const EventWall = ({
    ...
    const handleSubmitPost = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (axios.isAxiosError(error)) {
                alert(error.response?.data || 'Có lỗi xảy ra khi đăng bài');
            }
    ...
    const canPost = isAuthenticated &&
        ...
        (
            isPrivileged ||
            (isRegistered && isRegistrationApproved)
        );
    ...
    const getAccessMessage = () => {
        ...
        if (registrationStatus === 'PENDING') {
            return {
                ...
                message: 'Đăng ký đang đọc đang chờ duyệt. Bạn sẽ có thể đăng bài sau khi được quản lý sự kiện duyệt.'
            };
        }
        ...
        if (registrationStatus === 'REJECTED') {
            return {
                ...
                message: 'Đăng ký của bạn đã bị từ chối. Bạn không thể tham gia kênh trao đổi.'
            };
        }
        ...
        if (!isEventPublished) {
            return {
                ...
                message: 'Sự kiện đang chờ duyệt. Kênh trao đổi sẽ mở sau khi sự kiện được công bố.'
            };
        }
    ...
    return (
        <div className="mt-12">
            ...
            {/* Access message */}
            {accessMessage && (
                <div className={`alert ${alertTypeClass[accessMessage.type as keyof typeof alertTypeClass]} mb-6 shadow-lg`}>
                    {accessMessage.icon}
            ...
            <div className="space-y-4">
                {isLoading ? (
                    ...
                    posts.map((post) => (
                        <div key={post.id} className="card bg-base-100 border border-base-300 shadow-sm hover:shadow-md transition-shadow">
                            <div className="card-body">
                                <div className="flex items-center gap-3 mb-2">
                                    <div className="avatar placeholder w-8 h-8 relative rounded-full overflow-hidden">
                                        {post.author.imageUrl ? (
                                            <Image
                                                src={'/' + post.author.imageUrl.replace(/\\/g, '/').replace(/^\/+/, '')}
